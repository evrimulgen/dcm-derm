...
public class SampleDescriptor extends OperationDescriptorImpl implements
	...
	public RenderedImage create(ParameterBlock paramBlock,
		...
		return new SampleOpImage(paramBlock.getRenderedSource(0),
				new ImageLayout(), (Integer) paramBlock.getObjectParameter(0),
...
class SampleOpImage extends PointOpImage {
	...
	/**
	 * Constructs an SampleOpImage. The image dimensions are copied from the
	 ...
	 */
	public SampleOpImage(RenderedImage source, ImageLayout layout,
	 ...
	 * 
	 * @param sources
	 *            an array of sources, guarantee to provide all necessary source
	 *            data for computing the rectangle.
	 * @param dest
	 *            a tile that contains the rectangle to be computed.
	 * @param destRect
	 *            the rectangle within this OpImage to be processed.
	 */
	protected void computeRect(Raster[] sources, WritableRaster dest,
		...
		RasterAccessor srcAccessor = new RasterAccessor(sources[0], srcRect,
	...
	/**
	 * Computes an area of a given byte-based destination Raster using a souce
	 * RasterAccessor and a destination RasterAccesor. Processing is done as if
	 * the bytes are unsigned, even though the Java language has support only
	...
	/**
	 * Computes an area of a given int-based destination Raster using a source
	 * RasterAccessor and a destination RasterAccesor.
	 */
