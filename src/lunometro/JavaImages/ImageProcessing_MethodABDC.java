...
public class ImageProcessing_MethodABDC extends ImageProcessing{
	...
	public BufferedImage calcularABCD(Objeto mancha){
		...
		/*El valor de las Diferencias estructurales, D, depende del n�mero de estructuras que
		posee la lesi�n. Desde �reas homog�neas, cadenas, venas, puntos y/o gl�bulos, por lo
		que el valor de D var�a entre 1 y 5.*/
	...
	private int getCantColores(Objeto mancha) {
		...
		for(Pixel p : mancha.getPuntos()){
			...
			if(p.getCol().equals(Color.WHITE)){//Blanco
				...
				colorCantidad[1]++;
			}else if(hsvRange[0] > MIN_H_MARRON && hsvRange[0] < MAX_H_MARRON){//Marr�n Claro
				colorCantidad[2]++;
			}else if(p.getCol().equals(new Color(114, 35, 14))){//Marr�n oscuro
				colorCantidad[3]++;
			}else if(p.getCol().equals(new Color(30, 85, 83))){//Azul Gris�ceo,
				colorCantidad[4]++;
